...
const PlayerLifeState* snapshot_find_life(const WorldSnapshot& snapshot, EntityId entity) {
    const auto it = std::find_if(snapshot.player_life.begin(), snapshot.player_life.end(), [&](const PlayerLifeState& item) {
...
Result<std::string> encode_snapshot(const WorldSnapshot& snapshot) {
    ...
    for (const auto& dynamics : snapshot.player_dynamics) {
        ...
        if (dynamics.updated_world_minute > current_world_minute) {
            return Result<std::string>::failure(ErrorCode::ValidationFailed, "snapshot player dynamics references a future HOME minute");
        }
        if (std::find(dynamics_entities.begin(), dynamics_entities.end(), dynamics.entity) != dynamics_entities.end()) {
            return Result<std::string>::failure(ErrorCode::AlreadyExists, "duplicate snapshot player dynamics entity");
        }
        ...
        if (entity == nullptr || entity->kind != EntityKind::Avatar || life == nullptr) {
            return Result<std::string>::failure(ErrorCode::ValidationFailed, "snapshot player dynamics requires matching avatar life state");
        }
        if (dynamics.updated_world_minute < life->born_world_minute) {
            return Result<std::string>::failure(ErrorCode::ValidationFailed, "snapshot player dynamics predates player birth minute");
        }
        if (life->presence == LifePresence::Deceased && dynamics.autonomy.mode != AutonomyMode::Disabled) {
            return Result<std::string>::failure(ErrorCode::ValidationFailed, "snapshot deceased player retains active autonomy");
        }
    }
    ...
    for (const auto& dynamics : snapshot.player_dynamics) {
        ...
        out << ' ' << dynamics.mood.valence << ' ' << dynamics.mood.arousal << ' '
            << static_cast<unsigned>(dynamics.mood.band) << ' '
            << static_cast<unsigned>(dynamics.autonomy.mode) << ' '
            << dynamics.autonomy.initiative_limit_per_hour << ' '
            << (dynamics.autonomy.may_change_zone ? 1 : 0) << ' '
            << (dynamics.autonomy.may_interact_with_entities ? 1 : 0) << ' '
            << dynamics.updated_world_minute << ' ' << dynamics.sequence << ' '
            << std::quoted(dynamics.active_drive) << '\n';
    }
...
Result<WorldSnapshot> decode_snapshot(std::string_view encoded) {
    ...
    if (format >= 2) {
        ...
        if (!(in >> marker >> ratio >> time >> remainder) || marker != "CLOCK" || ratio == 0 || remainder >= ratio) {
            return parse_error("malformed snapshot clock state");
        }
    ...
    if (format >= 4) {
        ...
        for (std::size_t i = 0; i < count; ++i) {
            ...
            if (!(in >> marker >> id >> kind >> priority >> month >> day >> duration >> affinity_count
                  >> std::quoted(event.key) >> std::quoted(event.display_name))
                || marker != "V" || id == 0 || kind > static_cast<unsigned>(EventKind::Story)) {
    ...
    if (format >= 8) {
        ...
        for (std::size_t i = 0; i < count; ++i) {
            ...
            for (auto& value : dynamics.needs.values) {
                if (!(in >> value)) return parse_error("malformed player dynamics needs");
            }
            if (!(in >> dynamics.mood.valence >> dynamics.mood.arousal >> band >> mode >> initiative
                  >> may_change_zone >> may_interact >> updated >> sequence >> std::quoted(dynamics.active_drive))
                || band > static_cast<unsigned>(MoodBand::Elevated)
                || mode > static_cast<unsigned>(AutonomyMode::Bounded)
                || (may_change_zone != 0 && may_change_zone != 1)
                || (may_interact != 0 && may_interact != 1)) {
                return parse_error("malformed player dynamics record");
            }
            ...
            const auto valid = validate_player_dynamics_shape(dynamics);
            if (!valid) return parse_error("invalid player dynamics record");
            if (std::any_of(snapshot.player_dynamics.begin(), snapshot.player_dynamics.end(), [&](const PlayerDynamicsState& existing) {
                return existing.entity == dynamics.entity;
            })) {
                return parse_error("duplicate player dynamics entity");
            }
    ...
    for (std::size_t i = 0; i < count; ++i) {
        ...
        if (!(in >> marker >> id >> kind >> persistent >> std::quoted(e.archetype) >> std::quoted(e.display_name)
              >> e.transform.position.x >> e.transform.position.y >> e.transform.position.z
              >> e.transform.rotation.pitch >> e.transform.rotation.yaw >> e.transform.rotation.roll)
            || marker != "E" || id == 0 || kind > static_cast<unsigned>(EntityKind::Environment)) {
    ...
    for (std::size_t i = 0; i < count; ++i) {
        ...
        if (!(in >> marker >> id >> kind >> persistent >> parent >> std::quoted(z.key) >> std::quoted(z.display_name))
            || marker != "Z" || id == 0 || kind > static_cast<unsigned>(ZoneKind::Restricted)) {
            return parse_error("malformed zone record");
        }
        z.id = ZoneId{id};
        z.world = snapshot.world;
        z.kind = static_cast<ZoneKind>(kind);
    ...
    for (std::size_t i = 0; i < count; ++i) {
        ...
        std::string tag;
        if (!(in >> marker >> from >> to >> bidirectional >> traversable >> std::quoted(tag))
            || marker != "C" || from == 0 || to == 0) {
            return parse_error("malformed connection record");
        }
        snapshot.connections.push_back(ZoneConnection{
            ZoneId{from}, ZoneId{to}, bidirectional != 0, traversable != 0, std::move(tag)});
    }

    if (!(in >> marker >> count) || marker != "PLACEMENTS") return parse_error("malformed placement section");
    for (std::size_t i = 0; i < count; ++i) {
        std::uint64_t entity = 0, zone = 0;
        if (!(in >> marker >> entity >> zone) || marker != "P" || entity == 0 || zone == 0) {
            return parse_error("malformed placement record");
        }
    ...
    if (format >= 8) {
        ...
        for (const auto& dynamics : snapshot.player_dynamics) {
            ...
            const PlayerLifeState* life = snapshot_find_life(snapshot, dynamics.entity);
            if (entity == nullptr || entity->kind != EntityKind::Avatar || life == nullptr) {
                return parse_error("snapshot player dynamics requires matching avatar life state");
            }
            if (dynamics.updated_world_minute > current_world_minute || dynamics.updated_world_minute < life->born_world_minute) {
                return parse_error("snapshot player dynamics chronology is invalid");
            }
            if (life->presence == LifePresence::Deceased && dynamics.autonomy.mode != AutonomyMode::Disabled) {
                return parse_error("snapshot deceased player retains active autonomy");
            }
        }
    }
