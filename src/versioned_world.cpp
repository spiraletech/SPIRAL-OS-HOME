...
Result<EntityRecord> VersionedWorld::remove_entity(EntityId id) {
    ...
    for (const auto& skill : progression_purge.value().skills) changes.push_back({WorldChangeKind::SkillStateChanged, SkillStateChanged{skill, std::nullopt}});
    for (const auto& task : progression_purge.value().tasks) changes.push_back({WorldChangeKind::TaskStateChanged, TaskStateChanged{task, std::nullopt}});
    for (const auto& quest : progression_purge.value().quests) changes.push_back({WorldChangeKind::QuestStateChanged, QuestStateChanged{quest, std::nullopt}});
...
Result<TransactionReceipt> VersionedWorld::execute(const WorldTransaction& tx) {
    ...
    for (const auto& op : tx.operations) {
        ...
        std::visit([&](const auto& command) {
            ...
            if constexpr (std::is_same_v<T, TxCreateEntity>) {
                ...
                for(const auto& skill:progression.value().skills)changes.push_back({WorldChangeKind::SkillStateChanged,SkillStateChanged{skill,std::nullopt}});
                for(const auto& task:progression.value().tasks)changes.push_back({WorldChangeKind::TaskStateChanged,TaskStateChanged{task,std::nullopt}});
                for(const auto& quest:progression.value().quests)changes.push_back({WorldChangeKind::QuestStateChanged,QuestStateChanged{quest,std::nullopt}});
    ...
    registry_=std::move(staged_registry); topology_=std::move(staged_topology); player_life_=std::move(staged_life); player_dynamics_=std::move(staged_dynamics); relationships_=std::move(staged_relationships); inventory_=std::move(staged_inventory); progression_=std::move(staged_progression); const WorldRevision from=revision_; revision_=*next; history_.emplace_back(from,revision_,std::move(changes)); receipt.to_revision=revision_;
