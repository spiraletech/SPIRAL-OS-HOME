...
Result<EntityId> VersionedWorld::create_entity(EntityCreateInfo info) {
    ...
    if (!c) {
        registry_.remove(id);
        return Result<EntityId>::failure(c.error().code, c.error().message);
    }
...
Result<void> VersionedWorld::restore_entity(EntityRecord record) {
    ...
    const auto r = registry_.restore(std::move(record));
    if (!r) return r;
...
Result<EntityRecord> VersionedWorld::remove_entity(EntityId id) {
    ...
    ProgressionLedger staged_progression = progression_;
    ...
    const auto progression_purge = staged_progression.purge_player(id);
    if (!progression_purge) return Result<EntityRecord>::failure(progression_purge.error().code, progression_purge.error().message);
    const TheorismPurge theorism_purge = staged_theorism.purge_player(id);
    ...
    for (const auto& skill : progression_purge.value().skills) {
        changes.push_back({WorldChangeKind::SkillStateChanged, SkillStateChanged{skill, std::nullopt}});
    }
    for (const auto& task : progression_purge.value().tasks) {
        changes.push_back({WorldChangeKind::TaskStateChanged, TaskStateChanged{task, std::nullopt}});
    }
    for (const auto& quest : progression_purge.value().quests) {
        changes.push_back({WorldChangeKind::QuestStateChanged, QuestStateChanged{quest, std::nullopt}});
    }
    for (const auto& subclass : theorism_purge.subclasses) {
        changes.push_back({WorldChangeKind::SubclassAffinityStateChanged, SubclassAffinityStateChanged{subclass, std::nullopt}});
    }
    if (theorism_purge.aura) {
        changes.push_back({WorldChangeKind::AuraStateChanged, AuraStateChanged{theorism_purge.aura, std::nullopt}});
    }
    ...
    progression_ = std::move(staged_progression);
...
Result<void> VersionedWorld::update_transform(EntityId id, Transform transform) {
    EntityRecord* r = registry_.find_mutable(id);
    if (!r) return Result<void>::failure(ErrorCode::NotFound, "entity not found");
    if (r->transform == transform) return Result<void>::failure(ErrorCode::ValidationFailed, "transform update produced no state change");
    const Transform before = r->transform;
    r->transform = transform;
    const auto c = commit({{WorldChangeKind::EntityTransformUpdated, EntityTransformUpdated{id, before, transform}}});
    if (!c) {
        r->transform = before;
        return c;
    }
...
Result<ZoneId> VersionedWorld::create_zone(ZoneCreateInfo info) {
    const auto z = topology_.create_zone(std::move(info));
    if (!z) return z;
    const auto* r = topology_.find(z.value());
    if (!r) return Result<ZoneId>::failure(ErrorCode::InternalError, "created zone missing from topology");
    const auto c = commit({{WorldChangeKind::ZoneCreated, ZoneCreated{*r}}});
    if (!c) return Result<ZoneId>::failure(c.error().code, c.error().message);
    return z;
}

Result<void> VersionedWorld::restore_zone(ZoneRecord zone) {
    const auto copy = zone;
    const auto r = topology_.restore_zone(std::move(zone));
    if (!r) return r;
    return commit({{WorldChangeKind::ZoneCreated, ZoneCreated{copy}}});
}

Result<void> VersionedWorld::set_zone_parent(ZoneId child, std::optional<ZoneId> parent) {
    const auto* z = topology_.find(child);
    if (!z) return Result<void>::failure(ErrorCode::NotFound, "child zone not found");
    const auto before = z->parent;
    const auto r = topology_.set_parent(child, parent);
    if (!r) return r;
    const auto c = commit({{WorldChangeKind::ZoneParentChanged, ZoneParentChanged{child, before, parent}}});
    if (!c) {
...
Result<void> VersionedWorld::connect_zones(ZoneConnection connection) {
    ...
    const auto r = topology_.connect(std::move(connection));
    if (!r) return r;
    const auto c = commit({{WorldChangeKind::ZonesConnected, ZonesConnected{copy}}});
    if (!c) {
...
Result<void> VersionedWorld::place_entity(EntityId entity, ZoneId zone) {
    if (!registry_.contains(entity)) return Result<void>::failure(ErrorCode::NotFound, "entity not found");
    const auto before = topology_.zone_of(entity);
    const auto r = topology_.place_entity(entity, zone);
    if (!r) return r;
    const auto c = commit({{WorldChangeKind::EntityZoneChanged, EntityZoneChanged{entity, before, zone}}});
    if (!c) {
        if (before) topology_.place_entity(entity, *before);
...
Result<void> VersionedWorld::clear_entity_zone(EntityId entity) {
    if (!registry_.contains(entity)) return Result<void>::failure(ErrorCode::NotFound, "entity not found");
    const auto before = topology_.zone_of(entity);
    if (!before) return Result<void>::failure(ErrorCode::NotFound, "entity has no zone placement");
    const auto r = topology_.clear_entity(entity);
    if (!r) return r;
    const auto c = commit({{WorldChangeKind::EntityZoneChanged, EntityZoneChanged{entity, before, std::nullopt}}});
    if (!c) {
...
Result<TransactionReceipt> VersionedWorld::execute(const WorldTransaction& tx) {
    ...
    TheorismLedger staged_theorism = theorism_;
    std::vector<WorldChange> changes;
    TransactionReceipt receipt{tx.id, revision_, revision_, {}, {}};
    const std::uint64_t current_world_minute = clock_.now().milliseconds / 60000ULL;
    ...
    for (const auto& op : tx.operations) {
        ...
        std::visit([&](const auto& command) {
            ...
            if constexpr (std::is_same_v<T, TxCreateEntity>) {
                auto r = staged_registry.create(command.info);
                if (!r) { status = Result<void>::failure(r.error().code, r.error().message); return; }
                const auto* rec = staged_registry.find(r.value());
                receipt.created_entities.push_back(r.value());
                changes.push_back({WorldChangeKind::EntityCreated, EntityCreated{*rec}});
                ...
                const auto progression = staged_progression.purge_player(command.id);
                if (!progression) { status = Result<void>::failure(progression.error().code, progression.error().message); return; }
                const TheorismPurge theorism = staged_theorism.purge_player(command.id);

                auto r = staged_registry.remove(command.id);
                if (!r) { status = Result<void>::failure(r.error().code, r.error().message); return; }
                if (prior) {
                    staged_topology.clear_entity(command.id);
                    changes.push_back({WorldChangeKind::EntityZoneChanged, EntityZoneChanged{command.id, prior, std::nullopt}});
                }
                for (const auto& relationship : social.value().removed_relationships) {
                    changes.push_back({WorldChangeKind::RelationshipStateChanged, RelationshipStateChanged{relationship, std::nullopt}});
                }
                if (social.value().household_before) {
                ...
                for (const auto& task : progression.value().tasks) {
                    changes.push_back({WorldChangeKind::TaskStateChanged, TaskStateChanged{task, std::nullopt}});
                }
                for (const auto& quest : progression.value().quests) {
                    changes.push_back({WorldChangeKind::QuestStateChanged, QuestStateChanged{quest, std::nullopt}});
                }
                for (const auto& subclass : theorism.subclasses) {
                    changes.push_back({WorldChangeKind::SubclassAffinityStateChanged, SubclassAffinityStateChanged{subclass, std::nullopt}});
                }
                if (theorism.aura) {
                    changes.push_back({WorldChangeKind::AuraStateChanged, AuraStateChanged{theorism.aura, std::nullopt}});
                }
                if (prior_dynamics) {
                    const auto removed_dynamics = staged_dynamics.remove(command.id);
                    if (!removed_dynamics) { status = Result<void>::failure(removed_dynamics.error().code, removed_dynamics.error().message); return; }
                    changes.push_back({WorldChangeKind::PlayerDynamicsStateChanged, PlayerDynamicsStateChanged{prior_dynamics, std::nullopt}});
                }
                if (prior_life) {
                    const auto removed_life = staged_life.remove(command.id);
                    if (!removed_life) { status = Result<void>::failure(removed_life.error().code, removed_life.error().message); return; }
                    changes.push_back({WorldChangeKind::PlayerLifeStateChanged, PlayerLifeStateChanged{prior_life, std::nullopt}});
                }
                changes.push_back({WorldChangeKind::EntityRemoved, EntityRemoved{r.value()}});
            } else if constexpr (std::is_same_v<T, TxUpdateTransform>) {
                auto* rec = staged_registry.find_mutable(command.id);
                if (!rec) { status = Result<void>::failure(ErrorCode::NotFound, "entity not found"); return; }
                if (rec->transform == command.transform) { status = Result<void>::failure(ErrorCode::ValidationFailed, "transform update produced no state change"); return; }
                const auto before = rec->transform;
                rec->transform = command.transform;
                changes.push_back({WorldChangeKind::EntityTransformUpdated, EntityTransformUpdated{command.id, before, command.transform}});
            } else if constexpr (std::is_same_v<T, TxCreateZone>) {
                auto r = staged_topology.create_zone(command.info);
                if (!r) { status = Result<void>::failure(r.error().code, r.error().message); return; }
                const auto* rec = staged_topology.find(r.value());
                receipt.created_zones.push_back(r.value());
                changes.push_back({WorldChangeKind::ZoneCreated, ZoneCreated{*rec}});
            } else if constexpr (std::is_same_v<T, TxSetZoneParent>) {
                const auto* rec = staged_topology.find(command.child);
                if (!rec) { status = Result<void>::failure(ErrorCode::NotFound, "child zone not found"); return; }
                const auto before = rec->parent;
                auto r = staged_topology.set_parent(command.child, command.parent);
                if (!r) { status = r; return; }
                changes.push_back({WorldChangeKind::ZoneParentChanged, ZoneParentChanged{command.child, before, command.parent}});
            } else if constexpr (std::is_same_v<T, TxConnectZones>) {
                const ZoneConnection connection = command.connection;
                auto r = staged_topology.connect(connection);
                if (!r) { status = r; return; }
                changes.push_back({WorldChangeKind::ZonesConnected, ZonesConnected{connection}});
            } else if constexpr (std::is_same_v<T, TxPlaceEntity>) {
                if (!staged_registry.contains(command.entity)) { status = Result<void>::failure(ErrorCode::NotFound, "entity not found"); return; }
                const auto before = staged_topology.zone_of(command.entity);
                if (command.zone) {
                    auto r = staged_topology.place_entity(command.entity, *command.zone);
                    if (!r) { status = r; return; }
                } else {
                    if (!before) { status = Result<void>::failure(ErrorCode::NotFound, "entity has no zone placement"); return; }
                    auto r = staged_topology.clear_entity(command.entity);
                    if (!r) { status = r; return; }
                }
                changes.push_back({WorldChangeKind::EntityZoneChanged, EntityZoneChanged{command.entity, before, command.zone}});
            }
        }, op);
    ...
    const auto next = revision_.next();
    if (!next) return Result<TransactionReceipt>::failure(ErrorCode::Overflow, "world revision space exhausted");
    registry_ = std::move(staged_registry);
    topology_ = std::move(staged_topology);
    player_life_ = std::move(staged_life);
...
Result<std::vector<WorldDelta>> VersionedWorld::deltas_since(WorldRevision revision) const {
    if (revision > revision_) {
        return Result<std::vector<WorldDelta>>::failure(ErrorCode::RevisionConflict, "requested revision is ahead of canonical world revision");
    }
    std::vector<WorldDelta> out;
    for (const auto& delta : history_) if (delta.to_revision() > revision) out.push_back(delta);
    return Result<std::vector<WorldDelta>>::success(std::move(out));
}
